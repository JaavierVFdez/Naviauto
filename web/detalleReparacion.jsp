<%-- 
    Document   : detalleReparacion
    Created on : 8 jun 2023, 23:06:38
    Author     : javie
--%>
...
<html>
    <head>
        ...
        <title>Naviauto - Detalle Reparación</title>
        <!--Bootstrap-->
            ...
            <div class="table-responsive">
                <div class="table-wrapper">
                    <div class="table-title">
                        <div class="row">
                            <div class="col-sm-6">
                                <h2><a class="aMenu" style="color: white; text-decoration: none; margin-right: 1em;" href="Reparaciones"><svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-arrow-90deg-left" viewBox="0 0 16 16">
                                        <path fill-rule="evenodd" d="M1.146 4.854a.5.5 0 0 1 0-.708l4-4a.5.5 0 1 1 .708.708L2.707 4H12.5A2.5 2.5 0 0 1 15 6.5v8a.5.5 0 0 1-1 0v-8A1.5 1.5 0 0 0 12.5 5H2.707l3.147 3.146a.5.5 0 1 1-.708.708l-4-4z"/>
                                        </svg></a><b>Detalles de mi Reparación</b></h2>
                            </div>
                        </div>
                    </div>
                    <table class="table table-striped table-hover text-center">
                        <c:choose>
                            <c:when test="${empty reparacion}">
                                <h5>No hay reparaciones para mostrar.</h5>
                            </c:when>
                            <c:otherwise>
                                <thead>
                                    <tr>
                                        ...
                                        <!-- Verificar si la fecha de finalización tiene valor para mostrar encabezados -->
                                        <c:if test="${not empty reparacion.fechaF and reparacion.fechaF != ''}">
                                            <th>FECHA FINALIZACIÓN</th>
                                            <th>ESTADO</th>
                                            <th>PRECIO</th>
                                            </c:if>

                                        <!-- Si no hay fecha de finalización, mostrar solo el encabezado de estado -->
                                        <c:if test="${empty reparacion.fechaF or reparacion.fechaF == ''}">
                                            <th>ESTADO</th>
                                            </c:if>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        ...
                                        <td><c:out value="${reparacion.fechaE}"/></td>

                                        <!-- Verificar si la fecha de finalización tiene valor -->
                                        <c:if test="${not empty reparacion.fechaF and reparacion.fechaF != ''}">
                                            <td><c:out value="${reparacion.fechaF}" /></td>
                                            <td><c:out value="${reparacion.estado}"/></td>
                                            <td><c:out value="${reparacion.precio}" /> €</td>
                                        </c:if>

                                        <!-- Si no hay fecha de finalización, mostrar solo el estado -->
                                        <c:if test="${empty reparacion.fechaF or reparacion.fechaF == ''}">
                                            <td><c:out value="${reparacion.estado}"/></td>
                                        </c:if>
